...
case "$NODE_NUMBER" in
    ...
    2)
        ...
        ;;
    *)
        log_error "Opção inválida. Digite 1 ou 2."
        ;;
esac
...
# Instalar pip packages para IA distribuída
sudo -u "$REAL_USER" pip3 install --quiet --upgrade pip
...
sudo -u "$REAL_USER" pip3 install --quiet \
    ...
    numpy \
    huggingface_hub \
    2>/dev/null || log_warn "Alguns pacotes Python falharam, instale manualmente."

log_ok "Stack Python para IA instalado!"
