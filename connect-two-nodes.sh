...
# Desabilitar set -e temporariamente para não sair em erros esperados
set +e

# Forçar criação e ownership correto do diretório .ssh
rm -rf "$SSH_DIR"
mkdir -p "$SSH_DIR"
chown "${REAL_USER}:${REAL_USER}" "$SSH_DIR"
chmod 700 "$SSH_DIR"

# Gerar chave como ROOT e corrigir dono depois (método mais confiável)
...
# Corrigir permissões e dono
chown "${REAL_USER}:${REAL_USER}" "${SSH_DIR}/id_rsa" "${SSH_DIR}/id_rsa.pub" 2>/dev/null
...
log_ok "Par de chaves SSH gerado em: ${SSH_DIR}/id_rsa"
